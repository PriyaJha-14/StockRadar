...
import { usePortfolioStore } from "../store/portfolioStore"; // ✅ NEW
...
const StockDetailsScreen = () => {
  ...
  const { addHolding } = usePortfolioStore(); // ✅ NEW
  ...
  // ✅ NEW: Handle Buy Stock (Virtual Trading)
  const handleBuyStock = () => {
    ...
    Alert.prompt(
      `Buy ${symbol}`,
      `Enter number of shares to buy\n\nCurrent Price: $${quote.regularMarketPrice.toFixed(
        2
      )}\n\nNote: This is virtual trading (paper trading)`,
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: (quantity: any) => {
            ...
            if (qty > 0) {
              const cost = qty * quote.regularMarketPrice;
              addHolding({
                symbol,
              ...
              Alert.alert(
                "Success! 🎉",
                `Bought ${qty} shares of ${symbol} for $${cost.toFixed(
                  2
                )}\n\nCheck your Portfolio tab to view your holdings.`
              );
            } else {
            ...
            <View className="bg-white rounded-lg p-4 shadow-sm">
              {profile.companyOfficers.map((officer: any, index: number) => (
                <View
                  key={index}
                  className={`py-2 ${
                    index < profile.companyOfficers.length - 1
  ...
  return (
    <View className="flex-1">
      ...
      {/* ✅ NEW: Floating Action Buttons (AI Chat + Buy Stock) */}
      <View className="absolute z-[99] bottom-6 w-[90%] self-center">
        {/* Buy Stock Button */}
        <Pressable
          onPress={handleBuyStock}
          className="bg-green-600 rounded-full h-14 items-center justify-center shadow-lg flex-row px-4 mb-3"
        >
        ...
        <Pressable
          onPress={handleAskAi}
          className="bg-blue-600 rounded-full h-14 items-center justify-center shadow-lg flex-row px-4"
        >
