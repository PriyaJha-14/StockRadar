...
const WatchlistScreen = () => {
  ...
  const handleAnalyzePortfolio = () => {
    if (!quotes?.body || stocks.length === 0) {
      Alert.alert(
        "Nothing to analyze",
        "Add some stocks to your watchlist and wait for prices to load."
      );
      return;
    }

    // Build a summary string for the entire watchlist
    let summary = "Watchlist Overview:\n\n";
    summary += `Total stocks: ${stocks.length}\n\n`;
    ...
    stocks.forEach((symbol, index) => {
      ...
      summary += `${index + 1}. ${symbol} - ${
        quote.longName || quote.shortName || symbol
      }\n`;
      summary += `   Price: $${price.toFixed(2)}\n`;
      summary += `   Change: ${change >= 0 ? "+" : ""}${change.toFixed(
        2
      )} (${changePercent >= 0 ? "+" : ""}${changePercent.toFixed(2)}%)\n\n`;
    });

    summary += `Approx. total value (sum of prices): $${totalValue.toFixed(
      2
    )}\n`;
    summary +=
      "\nPlease analyze this watchlist in terms of diversification, sector exposure, risk level, and potential areas to research further.";

    // Use a special "watchlist" context for AI
    setContext({
      symbol: "WATCHLIST",
      currentPrice: totalValue,
      change: 0,
      percentChange: 0,
      companyName: "My Watchlist",
      sector: "Mixed",
      industry: "Multiple",
      volume: 0,
      marketCap: "N/A",
      ...
      const stockContext = formatStockForAi(
        {
          ...
          pctchange:
            (quote.regularMarketChangePercent ?? 0).toFixed(2) + "%" || "0",
          name: quote.longName,
          marketCap: quote.marketCap?.toString() || "N/A",
        },
        undefined
      );
        ...
        <View className="h-full px-4 pt-16">
          {stocks.length === 0 ? (
            <View className="flex-1 items-center justify-center p-4">
              ...
              <Text
                className="text-base text-center text-white/70"
                style={{ fontFamily: "RubikMedium" }}
              >
              ...
              >
                ...
                <View className="mb-4 bg-white/10 rounded-xl p-4 border border-white/20">
                  <Text
                    ...
                    style={{ fontFamily: "RubikMedium" }}
                  >
                    Get comprehensive AI analysis of your entire watchlist
                    including diversification, risk assessment, and market
                    outlook.
                  </Text>
